...
const DEFAULT_QUESTIONS = [
  "Can you confirm the date, time and location of this appointment?",
  "Is there anything I need to bring, complete or avoid before I attend?",
  "Who should I contact if I need to change the appointment?",
  "What happens after this appointment or referral step?",
  "Is any information missing from my letter that I should know before I travel?",
];
...
function buildPreparationNotes(lower: string): string[] {
  ...
  if (lower.includes("medicine") || lower.includes("medication")) {
    notes.push("Take the medication list mentioned in the letter.");
  }
...
function buildMissingList(
  ...
  if (!facts.date) missing.push("No clear appointment or referral date was found.");
  if (!facts.time && (lower.includes("appointment") || lower.includes("test"))) {
    missing.push("No clear appointment time was found.");
  }
  if (!facts.location && (lower.includes("appointment") || lower.includes("test"))) {
    missing.push("No clear appointment location was found.");
  }
  if (!facts.contact) missing.push("No clear contact phone number was found.");
  if (!facts.reference) missing.push("No booking, hospital or referral reference was found.");
...
function findReference(text: string): string | undefined {
  const match = text.match(
    /\b(?:booking reference|referral reference|hospital number|reference|ref|nhs number)\s*[:#-]?\s*([A-Z]{1,5}[-/]?[A-Z0-9]{3,}(?:[-/][A-Z0-9]+)?)\b/i,
  );
  return match?.[1]?.trim();
}
...
function findService(lines: string[], text: string): string | undefined {
  ...
  return findLine(lines, /(clinic|service|department|team|diagnostic|referral)/i);
}
...
function cleanLabelPrefix(value?: string): string | undefined {
  if (!value) return undefined;
  return value.replace(/^(appointment date|date|time|location|hospital number|booking reference|referral reference):\s*/i, "").trim();
}
...
function cleanDetails(value: unknown, fallback: AdminDetail[]): AdminDetail[] {
  ...
  for (const item of value) {
    ...
    cleaned.push({
      ...
      evidence: maybe.evidence ? String(maybe.evidence) : undefined,
...
function cleanChecklist(value: unknown, fallback: ActionItem[]): ActionItem[] {
  ...
  for (const item of value) {
    ...
    cleaned.push({
      task: String(maybe.task),
      reason: maybe.reason ? String(maybe.reason) : undefined,
